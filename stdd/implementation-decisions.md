...
- Bug fix (2026-01-13): Fixed alphabetical comparison in `FindNextSubdirInAll`, `FindNextSubdir`, and `FindNextDifference`. The original implementation used **exact match** comparison (`name == startAfter`) to determine where to start searching. When `startAfter` was a filename (e.g., "date.key") that wasn't in the subdirectory list (which only contains directories), the `started` flag never became true, causing all valid subdirectories to be skipped. Fix: Changed to **alphabetical comparison** (`name <= startAfter`) to skip entries that come before or equal to `startAfter`, even when `startAfter` is not present in the current list being searched. This enables proper cross-type searching where a file's position can inform subdirectory traversal.
...
## 38. Help Popup Implementation [IMPL:HELP_POPUP] [ARCH:HELP_WIDGET] [REQ:HELP_POPUP]

### Decision: Create a `help` package with a scrollable keystroke catalog popup based on `widget.ListBox`.
**Rationale:**
...
**Token Coverage** `[PROC:TOKEN_AUDIT]`:
- Source: `help/help.go`, `app/goful.go`, `main.go`
- Tests: Manual verification for now; future automated tests can reference `[REQ:HELP_POPUP]`

**Validation Evidence** `[PROC:TOKEN_VALIDATION]`:
- To be captured after implementation with `./scripts/validate_tokens.sh` output

**Cross-References**: [ARCH:HELP_WIDGET], [REQ:HELP_POPUP], [REQ:MODULE_VALIDATION]
...
## 38. Sync Execute [IMPL:SYNC_EXECUTE] [ARCH:SYNC_MODE] [REQ:SYNC_COMMANDS]

### Decision: Implement sync command mode with prefix key pattern and sequential execution
**Rationale:**
...
- **Add operation modes to `app/mode.go`**:
  - `syncCopyMode`: Prompts for new filename (default: original name), validates user enters a different name, stores `ignoreFailures`, copies file to new name in each pane's directory.
...
**Token Coverage** `[PROC:TOKEN_AUDIT]`:
- Source: `filer/directory.go`, `app/window_wide.go`, `app/mode.go`, `main.go`
- Tests: `app/window_wide_test.go` with test names referencing `REQ_SYNC_COMMANDS`

**Validation Evidence** `[PROC:TOKEN_VALIDATION]` (2026-01-14):
- `go test ./...` passes on darwin/arm64, Go 1.24.3
...
## 41. Mouse Double-Click Detection [IMPL:MOUSE_DOUBLE_CLICK] [ARCH:MOUSE_DOUBLE_CLICK] [REQ:MOUSE_DOUBLE_CLICK]

### Decision: Implement time-based double-click detection and action handlers for files and directories
**Rationale:**
...
**Test Coverage**:
- `app/mouse_test.go`: `TestIsDoubleClick_REQ_MOUSE_DOUBLE_CLICK` (table-driven timing/position tests), `TestDoubleClickThreshold_REQ_MOUSE_DOUBLE_CLICK` (threshold sanity check), `TestIsDoubleClickUpdatesState_REQ_MOUSE_DOUBLE_CLICK` (state updates), `TestDoubleClickSequence_REQ_MOUSE_DOUBLE_CLICK` (realistic click sequences)

**Token Coverage** `[PROC:TOKEN_AUDIT]`:
- Source: `app/goful.go`
- Tests: Unit tests with names referencing `REQ_MOUSE_DOUBLE_CLICK`

**Cross-References**: [ARCH:MOUSE_DOUBLE_CLICK], [REQ:MOUSE_DOUBLE_CLICK], [IMPL:MOUSE_FILE_SELECT], [REQ:LINKED_NAVIGATION], [REQ:MODULE_VALIDATION]

**Validation Evidence (2026-01-17)**:
- `go test ./app/... -run MOUSE_DOUBLE_CLICK` → 4 tests PASS
